...
def get_features(df):
    ...
    # Truncate raw emails to prevent massive memory usage and slow character loops
    docs = df['text'].apply(lambda x: x[:25000])
    ...
    # Count punctuation marks (optimized with regex instead of character loop)
    punctuation_pattern = re.compile(r"[^\w\s]")
    punctuation_counts = docs.apply(lambda x: len(punctuation_pattern.findall(x))) 
    ...
    # Count digits (optimized with regex)
    digit_pattern = re.compile(r"\d")
    digit_counts = docs.apply(lambda x: len(digit_pattern.findall(x)))
        
    # Count alphabetic chars (optimized with regex)
    alpha_pattern = re.compile(r"[a-zA-Z]")
    alpha_counts = docs.apply(lambda x: len(alpha_pattern.findall(x)))
    ...
    for doc in docs:
        ...
        # Count HTML Tags using fast regex instead of slow BeautifulSoup
        tag_counts.append(len(html_tag_pattern.findall(doc)))

        # Count e-mail addresses (e.g. ilug@linux.ie)
        doc_email = mail_pattern.sub("EMAILHERE",doc)
        mail_counts.append(doc_email.count('EMAILHERE'))
        
        # Count URLs (e.g. https://lists.sourceforge.net/lists/listinfo/razor-user)
        doc_url = url_pattern.sub("URLHERE",doc)    
        url_counts.append(doc_url.count('URLHERE'))
        
        # Count Twitter usernames (e.g. @username)
        doc_mention = mention_pattern.sub("MENTIONHERE",doc)    
        mention_counts.append(doc_mention.count('MENTIONHERE'))
        
        # Count hashtags (##weddingdress)
        doc_hashtag = hashtag_pattern.sub("HASHTAGHERE",doc)    
        hashtag_counts.append(doc_hashtag.count('HASHTAGHERE'))
...
def clean_corpus(df):
    ...
    for doc in docs:
        ...
        # Remove HTML Tags
        try:
            # Escape broken entity references like &#116er to prevent html.parser crashes in Python 3.14
            doc_cleaned = re.sub(r'&#(?![0-9]+;)', r'&amp;#', doc)
            soup = BeautifulSoup(doc_cleaned, 'html.parser')
            doc = soup.get_text()
        except Exception:
            doc = re.sub(r'<[^>]+>', '', doc)
        doc  = doc.replace('[\s]+',' ')
